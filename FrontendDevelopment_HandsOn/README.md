...
| 5 | React fundamentals (components, props, state, hooks) | `handson_05_06_10_react/` |
| 6 | React Router, Context API & Redux Toolkit | `handson_05_06_10_react/` (same project, extended) |
| 7 | Angular — components, services, DI, routing, forms | `handson_07_angular/` |
| 8 | Vue.js — Composition API, Vue Router & Pinia | `handson_08_vue/` |
| 9 | Accessibility & cross-browser compatibility | `handson_09/` (audits the Hands-On 1–3 HTML/CSS/JS portal) |
| 10 | Centralised API layer & advanced Redux (framework chosen: **React**) | `handson_05_06_10_react/` (same project, extended) |

Hands-On 5, 6, and 10 share a single `handson_05_06_10_react/` project because they
are designed to build on each other (per the exercise book's "single running project"
note). Each hands-on's specific screenshots are still separated inside
`handson_05_06_10_react/screenshots/` with a `handson5_`, `handson6_`, or `handson10_`
filename prefix.
...
- **handson_05_06_10_react/**: `npm install && npm run dev` → http://localhost:5173
- **handson_07_angular/**: `npm install && npx ng serve` → http://localhost:4200
- **handson_08_vue/**: `npm install && npm run dev` → http://localhost:5174 (or
